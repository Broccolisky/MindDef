...
| 用户名 | 密码 | 说明 |
|--------|------|------|
| `admin` | `123456` | 管理员（预置 8 条任务，覆盖四象限 + 3 种状态） |
| `demo` | `123456` | 演示用户（预置 5 条任务） |
...
| PUT | `/api/task/{id}` | 编辑任务（内容/重要性/紧急性/截止日期/来源/重复模式） |
...
| `task` | 任务表 | 13 | id, user_id, content, importance, urgency, quadrant, deadline, source, repeat_mode, status |
...
> - 拖拽区域改造：番茄钟（上方红色）+ 完成（下方绿色左）/ 删除（下方红色右）
> - 新建/编辑页新增重复模式芯片（每日 / 仅工作日 / 仅休息日）
> - 编辑接口扩展：支持修改 importance/urgency/deadline/source/repeatMode
...
> - GuideCreatePage "微调"跳转显示 "？·"（replaceUrl → pushUrl）
> - TaskService.java 重复完成无拦截（增加 status=2 校验）
> - AiService.java 频率限制非线程安全（ConcurrentLinkedDeque）
> - FocusPage 后台挂起不结束专注（onPageHide 调用 doEnd）
...
**Q: 如何部署到服务器？**  
A: 详见 `deploy/README.md`。后端使用 systemd 管理：`systemctl restart minddef` 重启，`journalctl -u minddef -f` 查看日志。服务器 Swagger：http://47.239.55.169:8080/swagger-ui.html
